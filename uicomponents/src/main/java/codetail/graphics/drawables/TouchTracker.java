...
public class TouchTracker implements View.OnTouchListener{
    ...
    public boolean onTouch(View v, MotionEvent event) {
        if(!(v.getBackground() instanceof HotspotDrawable)){
            return false;
        }

        final float x = event.getX();
        final float y = event.getY();
        final HotspotDrawable hotspotDrawable = (HotspotDrawable) v.getBackground();
        ...
        if(v.isClickable() || v.isLongClickable()) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    hotspotDrawable.setHotspot(x, y);
    ...
    void setPressed(View target, boolean pressed, float x, float y){
        ...
        HotspotDrawable hotspotDrawable = (HotspotDrawable) target.getBackground();
        hotspotDrawable.setHotspot(x, y);
    }
